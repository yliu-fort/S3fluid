...
class Mesh(object):
    def __init__(self, points, simplices, point_normals=None) -> None:
        ...
        directions = np.where(np.sum((self.edge_centers - self.barycenters[:, self.owners]) * self.edge_normals, axis=0) > 0, 1, -1)
        ...
        self.d_cf = np.linalg.norm(self.cf, axis=0)
        self.e = self.cf / self.d_cf

        # Compute ef
        self.ef = self.edge_lengths / np.sum(self.e * self.edge_normals, axis=0)
        ...
        # Compute Tf
        self.normal_dot_e = np.sum(self.edge_normals * self.e, axis=0)
        self.tf = (self.edge_normals - self.e / self.normal_dot_e) * self.edge_lengths
    ...
    def interpolate_field_cell_to_face(self, phi):
        return (1.0-self.edge_weighing_factor) * phi[...,self.owners] + \
    ...
    def reconstruct_surface_gradient(self, phi, n_skewness_corr_iter=1, dirty=False):
        ...
        for _ in range(1+n_skewness_corr_iter):
            ...
            # Correct phi_f
            phi_f += np.sum(dphi_f * self.skewness_vector, axis=0)
    ...
    def convection_matrix(self, phi, rhou, quick=False, face_flux=False, dirty=False):
        ...
        if face_flux:
            ...
            rhou_f = (1.0 - self.edge_weighing_factor) * rhou[...,self.owners] + \
                         np.where(self.neighbours != -1, self.edge_weighing_factor * rhou[...,self.neighbours], np.zeros_like(rhou[...,self.owners]))
        ...
        if dirty or self.convection_operator == {}:
            ...
            # Compute data values
            val = np.sum(rhou_f * self.edge_normals, axis=0) * self.edge_lengths
            val = val[valid_faces]
        ...
        if quick:
            np.add.at(self.convection_operator["rhs"], self.owner_indices,     -val_p * 0.5 * np.sum((dphi[...,self.owner_indices]+dphi_f[...,valid_faces]) * (self.edge_centers[...,valid_faces] - self.barycenters[...,self.owner_indices]), axis=0))
            np.add.at(self.convection_operator["rhs"], self.neighbour_indices,  val_p * 0.5 * np.sum((dphi[...,self.owner_indices]+dphi_f[...,valid_faces]) * (self.edge_centers[...,valid_faces] - self.barycenters[...,self.owner_indices]), axis=0))
            np.add.at(self.convection_operator["rhs"], self.owner_indices,     -val_n * 0.5 * np.sum((dphi[...,self.neighbour_indices]+dphi_f[...,valid_faces]) * (self.edge_centers[...,valid_faces] - self.barycenters[...,self.neighbour_indices]), axis=0))
            np.add.at(self.convection_operator["rhs"], self.neighbour_indices,  val_n * 0.5 * np.sum((dphi[...,self.neighbour_indices]+dphi_f[...,valid_faces]) * (self.edge_centers[...,valid_faces] - self.barycenters[...,self.neighbour_indices]), axis=0))
    ...
    def helmholtz_projection(self, mass_flow_rate, dirty=False):
        ...
        if dirty or self.possion_operator == {}:
            ...
            # Create sparse matrix
            Jf = csr_matrix((data_values,(row_indices,col_indices)), shape=(len(self.areas), len(self.areas)))
        ...
        self.possion_operator["dphi_fn"][valid_faces] = (phi[...,self.neighbour_indices] - phi[...,self.owner_indices]) / self.d_cf[valid_faces] / self.normal_dot_e[valid_faces]
