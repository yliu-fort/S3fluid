...
points0, simplices0 = icosphere(30)
points, simplices = dual_grid(points0, simplices0)
...
mesh0 = meshio.Mesh(
    points0,
    [("triangle", simplices0),],
    point_data={"u": u,"v": v,"c": np.sqrt(gamma)},
)
mesh0.write(
...
with meshio.xdmf.TimeSeriesWriter("wave_test.xdmf") as writer:
    writer.write_points_cells(points0, [("triangle", simplices0),])
    for t in range(1000+1):
        ...
        print(t*dt)
        if t % 10 == 0:
            writer.write_data(t*dt, point_data={"u": u,"v": v,"c": np.sqrt(gamma)})
