...
# %%
output_path = "results/diffusion_test.xdmf"
cli_utils.ensure_directory(output_path)

with meshio.xdmf.TimeSeriesWriter(output_path) as writer:
    writer.write_points_cells(points, [("triangle", simplices),])
    for t in cli_utils.progress_bar(range(101), description="Simulating Diffusion"):
        # Solve
        dt = 0.01
